# Comprehensive installer for cognis-digital/jwtinspect (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# jwtinspect is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "jwtinspect"
$Url  = "git+https://github.com/cognis-digital/jwtinspect.git"
$Git  = "https://github.com/cognis-digital/jwtinspect.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: jwtinspect"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: jwtinspect"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: jwtinspect"; exit 0 }
}
